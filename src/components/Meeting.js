...
class Meeting extends React.Component {
    ...
    componentDidMount() { 
        ...
        socket.on("newParticipantB", participantID => {
            ...
            this.props.dispatch(newParticipant({
                id:participantID,
    ...
    render() {
        ...
        if(this.state.agendaList.length>0){
            agenda = this.state.agendaList.map((item) => {
                return (
                    <li id='agenda-li'>{item}<button onClick={()=>this.onClick_deleteAgenda(item)} id='cancel'>刪除</button></li>
                )
            });
        }        
        ...
        if(this.state.textRecord.length>0){
            this.state.textRecord.map((obj)=>{
                if(obj.userID == this.localUserID){
                    chat.push(
                        <div id="me_sent">
                            <div className="arrow_box4"><div id="me-text">{obj.text}</div></div>
            ...
            <div>
                ...
                <div className="box-b">
                    <div id="meet_chat">
                        ...
                        <div id="yourvoice">
                            <img id="voice_img" src={this.state.isRecognizing? "../img/mic-animate.gif":"../img/mic.gif"} />
                            {this.state.recognitionResult}
                        </div>
                        <div id="meet_chat_input">
                            <textarea
                                onKeyPress={(e)=>{this.handleSendText_Enter(e)}}
                                id="meet_input"
                                ref="meet_input"
                            />
                            <button
                                ...
                                ref="meet_submit"
                                maxLength="25"
                                onClick={()=>{this.handleSendText_Click()}}
                            >
                                送出
                            </button>
                        </div>
                    ...
                    <div id="feature">
                        <div className="left">
                            <button
                                id={this.state.isRecognizing? "recognize-on":"recognize-off"}
                        ...
                        <div className="center">
                            ...
                            <button
                                id={this.state.isAgendaOpen? "agenda-off":"agenda-on"}
                    ...
                    <div id="meet_main" ref="meet_main">
                        ...
                        <div id={this.state.isRecognizing? "recognition_detail_off":"recognition_detail_on"}>
                            <select
                                name="language"
                                id="language"
                                ref="select_language"
                            ...
                            <select
                                ...
                                id="dialect"
                                ref="select_dialect"
                                onClick={(e)=>{this.Recognizer.setLanguage(e.target)}}
                            />
                        </div>

                        <div id={this.state.isinviteOpen? "invite_detail_on":"invite_detail_off"}>
                            <div id="meetpage">網址：</div>
                            <textarea id="pagetext" value={this.state.roomURL}></textarea>
                        </div>
                        <div id={this.state.isSounding? "VideoUser-audio-on":"VideoUser-audio-off"}>
                            <video
                                id="videoSrc"
                                width="220"
                                muted="muted"
                                src={this.state.videoIsReady && this.state.isStreaming ? this.state.localVideoURL: "沒在播放欸或是沒加到影像"}
                                autoPlay={true}
                            />
                        </div>
                        {remoteVideo}

                        <div id={this.state.isAgendaOpen? "nowagenda-on":"nowagenda-off"}>
                        ...
                        <div id={this.state.isBrainstormingOpen? "brainbox-on":"brainbox-off"}>
                            ...
                            <div id="BrainHat">
                                你目前所戴的帽子<br /><img src={"../img/"+this.state.hatColor[0]+".png"} />
                            </div>
                            <div id="BarinHatText">
                                <ul>{brainSupport}</ul>
                            </div>
...
const mapStateToProps = state => {
    return {
        chatList:state.meeting.chatList,
